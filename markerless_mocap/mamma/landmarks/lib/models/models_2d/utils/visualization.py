...
def compare_results_denseldmks2d(images, pred_vals, targets, masks, iteration, path, normalize_plus_min_one=False):
    ...
    preds = to_numpy_safe(pred_vals["joints2d"]) #.detach().cpu().numpy()
    visibility = to_numpy_safe(torch.sigmoid(pred_vals["visibility"]).squeeze(-1)) if "visibility" in pred_vals else None
    visibility_gt = to_numpy_safe(targets["joints_visibility"].squeeze(-1)) if "visibility" in pred_vals else None
    contact = to_numpy_safe(torch.sigmoid(pred_vals["contact"]).squeeze(-1)) if ("contact" in pred_vals and pred_vals["contact"] is not None) else None
    contact_gt = to_numpy_safe(targets["joints_contact"].squeeze(-1)) if ("contact" in pred_vals and pred_vals["contact"] is not None) else None
    floor_contact = to_numpy_safe(torch.sigmoid(pred_vals["floor_contact"]).squeeze(-1)) if ("floor_contact" in pred_vals and pred_vals["floor_contact"] is not None) else None
    floor_contact_gt = to_numpy_safe(targets["joints_floor_contact"].squeeze(-1)) if ("floor_contact" in pred_vals and pred_vals["floor_contact"] is not None) else None
    ...
    for i, (image, pred, target, mask) in enumerate(zip(images, preds, targets["joints"], masks)):
        ...
        contact_image = image.copy()
        gt_contact_image = image.copy()
        floor_contact_image = image.copy()
        gt_floor_contact_image = image.copy()
        mask_image = None
        ...
        contact_image = cv2.putText(contact_image, "Contact ldmks", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 0), 1)
        gt_contact_image = cv2.putText(gt_contact_image, "GT Contact ldmks", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 0), 1)
        floor_contact_image = cv2.putText(floor_contact_image, "Floor Contact ldmks", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 0), 1)
        gt_floor_contact_image = cv2.putText(gt_floor_contact_image, "GT Floor Contact ldmks", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 0), 1)
        ...
        for j, (pred_point, target_point) in enumerate(zip(pred, target)):
            ...
            if contact is not None:
                contact_diff = np.abs(contact[i,j] - contact_gt[i,j])
                if contact_gt[i,j] > 0.5:
                    # green contact / red no contact
                    contact_image = cv2.circle(contact_image, (int(target_x), int(target_y)), circle_size,
                                        (0, int(255*(1-contact_diff)), int(255*contact_diff)), -1)
                else:
                    gt_contact_image = cv2.circle(gt_contact_image, (int(target_x), int(target_y)), circle_size,
                                            (0, int(255*(1-contact_diff)), int(255*contact_diff)), -1)
            if floor_contact is not None:
                floor_contact_diff = np.abs(floor_contact[i,j] - floor_contact_gt[i,j])
                if floor_contact_gt[i,j] > 0.5:
                    # green contact / red no contact
                    floor_contact_image = cv2.circle(floor_contact_image, (int(target_x), int(target_y)), circle_size,
                                        (0, int(255*(1-floor_contact_diff)), int(255*floor_contact_diff)), -1)
                else:
                    gt_floor_contact_image = cv2.circle(gt_floor_contact_image, (int(target_x), int(target_y)), circle_size,
                                            (0, int(255*(1-floor_contact_diff)), int(255*floor_contact_diff)), -1)

        image = np.concatenate((image, target_image, pred_image), axis=1)
        if visibility is not None:
            image = np.concatenate((image, vis_image, gt_vis_image), axis=1)

        if contact is not None:
            image = np.concatenate((image, contact_image, gt_contact_image), axis=1)

        if floor_contact is not None:
            image = np.concatenate((image, floor_contact_image, gt_floor_contact_image), axis=1)

        if mask is not None:
            image = np.concatenate((image, mask_image), axis=1)

        # reduce if height too large
        if image.shape[0] > 720:
            image = cv2.resize(image, (image.shape[1]//2, image.shape[0]//2))

        print("video image size", image.shape)
        writer.append_data(cv2.cvtColor(image, cv2.COLOR_BGR2RGB))
    writer.close()
